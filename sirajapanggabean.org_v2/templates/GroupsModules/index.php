<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\GroupsModule[]|\Cake\Collection\CollectionInterface $groupsModules
 */
?>
<div class="groupsModules index content">
    <?= $this->Html->link(__('New Groups Module'), ['action' => 'add'], ['class' => 'button float-right']) ?>
    <h3><?= __('Groups Modules') ?></h3>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('id') ?></th>
                    <th><?= $this->Paginator->sort('group_id') ?></th>
                    <th><?= $this->Paginator->sort('module_id') ?></th>
                    <th><?= $this->Paginator->sort('allowed') ?></th>
                    <th><?= $this->Paginator->sort('created') ?></th>
                    <th><?= $this->Paginator->sort('user_created') ?></th>
                    <th><?= $this->Paginator->sort('ip_created') ?></th>
                    <th><?= $this->Paginator->sort('modified') ?></th>
                    <th><?= $this->Paginator->sort('user_modified') ?></th>
                    <th><?= $this->Paginator->sort('ip_modified') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($groupsModules as $groupsModule): ?>
                <tr>
                    <td><?= $this->Number->format($groupsModule->id) ?></td>
                    <td><?= $groupsModule->has('group') ? $this->Html->link($groupsModule->group->name, ['controller' => 'Groups', 'action' => 'view', $groupsModule->group->id]) : '' ?></td>
                    <td><?= $groupsModule->has('module') ? $this->Html->link($groupsModule->module->name, ['controller' => 'Modules', 'action' => 'view', $groupsModule->module->id]) : '' ?></td>
                    <td><?= h($groupsModule->allowed) ?></td>
                    <td><?= h($groupsModule->created) ?></td>
                    <td><?= h($groupsModule->user_created) ?></td>
                    <td><?= h($groupsModule->ip_created) ?></td>
                    <td><?= h($groupsModule->modified) ?></td>
                    <td><?= h($groupsModule->user_modified) ?></td>
                    <td><?= h($groupsModule->ip_modified) ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('View'), ['action' => 'view', $groupsModule->id]) ?>
                        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $groupsModule->id]) ?>
                        <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $groupsModule->id], ['confirm' => __('Are you sure you want to delete # {0}?', $groupsModule->id)]) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')) ?></p>
    </div>
</div>
